...
for (p,po,m),g in df.groupby(["Periodo","Periodo_ord","Maquina"]):
    ...
    capacidade.append({"Periodo":p,"Periodo_ord":po,"Maquina":m,"Capacidade (h)":round(total)})
...
df_final["Ocupação (%)"] = df_final["Ocupação (%)"].replace([np.inf,-np.inf],0).fillna(0).round(0)
